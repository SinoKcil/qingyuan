...
<script setup lang='ts'>
   ...
   // var buttonString:String='border-radius:0%,height:50px,width:50px,background-color:'+choosedColor.value+',border-width:1px'
    ...
    function overMouse(indexRow,indexCol){//等于-1的时候执行前面那个，是通常情况
    ...
    function chooseColor(indexRow,indexCol){
        ...
        if(tableValue[indexRow][indexCol]==0&&activeCol.value==indexCol&&activeRow.value==indexRow) return greenButton
        if(tableValue[indexRow][indexCol]==0) return pinkButton
        if(tableValue[indexRow][indexCol]==1&&activeCol.value==indexCol&&activeRow.value==indexRow) 
        {
            showTip.value=true
            return darkredButton
        }
        if(tableValue[indexRow][indexCol]==1) return redButton
    }
    function routerToTable(category,ticketid){
        //页面显示有bug，无法直接展示，故暂时采用query（get）模式传参
        if(category=='detail') router.push({name:'FaultDetail',query:{Row:activeRow.value,Col:activeCol.value}})
        if(category=='ticket') router.push({name:'404',query:{ticketId:ticketid}})
    }
...
<!-- 测试卡片 -->
    ...
    <div style="flex-direction: column;grid-area:a">
        <el-card  class="card" shadow="never">
            ...
            <br>
            <div v-for="(col,indexRow) in myTable" :key="indexRow">
                <button v-for="(item,indexCol) in myTable[indexRow]" :key="indexCol"
                :style="chooseColor(indexRow,indexCol)" 
                @mouseover="overMouse(indexRow,indexCol)" @mouseleave="leaveMouse()" @click="routerToTable('detail',-1)">
                    {{ item }}
                </button>
            </div>
        </el-card>
    ...
    <div style="grid-area:b" class="issues">
        <el-card>
            ...
            <el-scrollbar max-height="200" class="mt-3">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" viewBox="0 0 24 24" class="inline-block">
                        <path fill="currentColor" :d="thisWrench==index?blackWrench:whiteWrench"/>
                    </svg>
                    <span class="text-text_color_regular text-sm inline-block"
                        @mouseover="thisWrench=index" @mouseleave="thisWrench=-1" @click="routerToTable('ticket',index+item)">
                        {{
    ...
    <div class="tooltip" :style="{left:cursorX+'px',top:cursorY+'px'}" v-show="showTip&&mobile">123</div>
</div>
</template>
